...
final readonly class DatabaseSiteSettingsProvider implements SiteSettingsProvider
{
    ...
    private function websiteSettings(int $siteId, array $site, string $language): array
    {
        ...
        return [
            ...
            $this->setting('sitesearch', $this->translator->translate('SitesManager_SiteSearchUse', $language), (int) ($site['sitesearch'] ?? 1), 1, 'integer', 'select', availableValues: [0 => $no, 1 => $yes]),
            $this->setting('use_default_site_search_params', $this->translator->translate('SitesManager_UseDefaultSiteSearchParams', $language), $list($site['sitesearch_keyword_parameters'] ?? '') === [], true, 'boolean', 'checkbox'),
            $this->setting('sitesearch_keyword_parameters', $this->translator->translate('SitesManager_SearchKeywordLabel', $language), $list($site['sitesearch_keyword_parameters'] ?? ''), [], 'array', 'text', condition: 'sitesearch==1 && use_default_site_search_params==0'),
            $this->setting('sitesearch_category_parameters', $this->translator->translate('SitesManager_SearchCategoryLabel', $language), $list($site['sitesearch_category_parameters'] ?? ''), [], 'array', 'text', condition: 'sitesearch==1'),
            $this->setting('ecommerce', $this->translator->translate('Goals_Ecommerce', $language), (int) ($site['ecommerce'] ?? 0), 0, 'integer', 'select', availableValues: [0 => $no, 1 => $yes]),
        ];
    }
