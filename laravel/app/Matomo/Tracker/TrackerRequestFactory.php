...
final readonly class TrackerRequestFactory
{
    ...
    public function make(Request $request): TrackingRequest
    {
        ...
        if (count(array_filter([$eventCategory, $download, $outlink, $search], static fn (?string $value): bool => $value !== null)) > 1) {
            throw new InvalidArgumentException('Only one tracker action type may be provided.');
        }

        $actionType = $eventCategory !== null ? 10 : ($download !== null ? 3 : ($outlink !== null ? 2 : ($search !== null ? 8 : 1)));
