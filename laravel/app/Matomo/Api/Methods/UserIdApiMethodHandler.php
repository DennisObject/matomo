...
use App\Matomo\Reporting\UserIdReportBuilder;
...
final readonly class UserIdApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private UserIdReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isUserIdRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The UserId API handler does not support this request.');
        }

        $query = $request->visitsSummary ?? throw new LogicException('The UserId request was not parsed.');
