...
use App\Matomo\Reporting\ContentsReportBuilder;
...
final readonly class ContentsApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private ContentsReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isContentsRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The Contents API handler does not support this request.');
        }

        $query = $request->visitsSummary ?? throw new LogicException('The Contents request was not parsed.');
        ...
        $report = $this->reports->build(
            byName: $request->method === 'Contents.getContentNames',
