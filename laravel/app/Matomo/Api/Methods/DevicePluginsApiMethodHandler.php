...
use App\Matomo\Reporting\DevicePluginsReportBuilder;
...
final readonly class DevicePluginsApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private DevicePluginsReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isDevicePluginsRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The DevicePlugins API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The DevicePlugins request was not parsed.');
