...
use App\Matomo\Reporting\VisitTimeReportBuilder;
...
final readonly class VisitTimeApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private VisitTimeReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isVisitTimeRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The VisitTime API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The VisitTime request was not parsed.');
