...
use App\Matomo\Reporting\ReferrersCampaignReportBuilder;
...
final readonly class ReferrersCampaignApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private ReferrersCampaignReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isReferrersCampaignRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The Referrers campaign API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The Referrers campaign request was not parsed.');
