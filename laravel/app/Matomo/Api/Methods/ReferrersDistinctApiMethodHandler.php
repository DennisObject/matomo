...
use App\Matomo\Reporting\ReferrersDistinctReportBuilder;
...
final readonly class ReferrersDistinctApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private ReferrersDistinctReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isReferrersDistinctRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The Referrers distinct API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The Referrers distinct request was not parsed.');
        ...
        if ($request->format !== 'rss') {
            return $this->responses->metricReport($request, $report);
        }
    ...
    private function rssResponse(
        ...
        try {
            return $this->responses->rss($this->rss->metric(
                $report,
                $periods,
                $idSite,
                $requestedPeriod,
