...
use App\Matomo\Reporting\ReferrersSearchReportBuilder;
...
final readonly class ReferrersSearchApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private ReferrersSearchReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isReferrersSearchRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The Referrers search API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The Referrers search request was not parsed.');
