...
use App\Matomo\Reporting\PagePerformanceReportBuilder;
...
final readonly class PagePerformanceApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private PagePerformanceReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isPagePerformanceRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The PagePerformance API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The PagePerformance request was not parsed.');
