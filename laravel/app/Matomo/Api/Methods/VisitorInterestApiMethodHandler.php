...
use App\Matomo\Reporting\VisitorInterestReportBuilder;
...
final readonly class VisitorInterestApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private VisitorInterestReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isVisitorInterestRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The VisitorInterest API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The VisitorInterest request was not parsed.');
