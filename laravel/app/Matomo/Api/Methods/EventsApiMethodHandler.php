...
use App\Matomo\Reporting\EventsReportBuilder;
...
final readonly class EventsApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private EventsReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isEventsRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The Events API handler does not support this request.');
        }

        $query = $request->visitsSummary ?? throw new LogicException('The Events request was not parsed.');
