...
use App\Matomo\Reporting\VisitFrequencyReportBuilder;
...
final readonly class VisitFrequencyApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private VisitFrequencyReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isVisitFrequencyRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The VisitFrequency API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The VisitFrequency request was not parsed.');
