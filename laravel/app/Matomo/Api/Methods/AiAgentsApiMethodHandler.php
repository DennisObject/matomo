...
use App\Matomo\Reporting\AiAgentsReportBuilder;
...
final readonly class AiAgentsApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private AiAgentsReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isAiAgentsRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The AIAgents API handler does not support this request.');
        }

        $query = $request->visitsSummary ?? throw new LogicException('The AIAgents request was not parsed.');
