...
use App\Matomo\Reporting\UserLanguageReportBuilder;
...
final readonly class UserLanguageApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private UserLanguageReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isUserLanguageRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The UserLanguage API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The UserLanguage request was not parsed.');
        ...
        $report = $this->reports->build(
            groupLanguages: $request->method === 'UserLanguage.getLanguage',
            siteIds: $siteIds,
            periods: $periods,
