...
use App\Matomo\Reporting\DevicesDetectionReportBuilder;
...
final readonly class DevicesDetectionApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private DeviceModelPolicy $modelPolicy,
        private DevicesDetectionReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isDevicesDetectionRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The DevicesDetection API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The DevicesDetection request was not parsed.');
        ...
        if ($request->method === 'DevicesDetection.getModel') {
            $filteredSiteIds = array_values(array_filter(
                $siteIds,
                fn (int $idSite): bool => ! $this->modelPolicy->detectionDisabled($idSite),
            ));

            if ($filteredSiteIds === []) {
                return $this->responses->error(
                    $request,
                    'Device model report is disabled by compliance policy.',
                    400,
                );
            }
