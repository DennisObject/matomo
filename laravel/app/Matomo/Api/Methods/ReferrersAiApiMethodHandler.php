...
use App\Matomo\Reporting\ReferrersAiReportBuilder;
...
final readonly class ReferrersAiApiMethodHandler implements ApiMethodHandler
{
    public function __construct(
        ...
        private ReferrersAiReportBuilder $reports,
        private RssReportRenderer $rss,
    ) {}

    public function supports(ApiRequest $request): bool
    {
        return $request->isReferrersAiRequest();
    }

    public function handle(ApiRequest $request, Request $httpRequest): Response
    {
        if (! $this->supports($request)) {
            throw new LogicException('The Referrers AI API handler does not support this request.');
        }

        $query = $request->visitsSummary
            ?? throw new LogicException('The Referrers AI request was not parsed.');
