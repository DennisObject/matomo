...
| --- | --- | ---: | --- |
| Reporting API methods | `plugins/*/API.php` | 389 methods | 378 method names handled |
...
- `UsersManager`: access-role and plugin-capability metadata, including localized role labels,
  ...
  The role-enriched user directory preserves anonymous and self-only behavior, shared-admin-site scope,
  invite filters, access filters, search, pagination totals, and sensitive-field redaction.
  Access mutations preserve site-admin boundaries, atomic multi-site role and capability writes,
  protected superuser access, anonymous restrictions and notifications, password confirmation for
  sensitive session grants, access-removal events, and tracker-cache invalidation.
  ...
  Superuser access changes and forced logout preserve superuser authorization, anonymous-account
  protection, password confirmation, last-superuser safety, atomic access cleanup, tracker-cache
  invalidation, user existence checks, and targeted session invalidation.
